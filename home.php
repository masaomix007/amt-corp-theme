                    ...
                    <?php if ($blog_query->have_posts()) : ?>
                        <?php while ($blog_query->have_posts()) : $blog_query->the_post(); ?>
                            <article class="flex flex-col md:flex-row gap-4 md:gap-8 border-b border-gray-300 pb-6 md:pb-10 last:border-b-0">
                                <a href="<?php the_permalink(); ?>" class="relative block w-full md:w-[280px] flex-shrink-0 group overflow-hidden">
                                    <?php 
                                    // ▼ NEWマーク：投稿から7日以内なら表示
            ...
            <aside class="w-full space-y-12 md:space-y-16 mt-10 md:mt-0 lg:px-4">
                ...
                <div>
                    <form role="search" method="get" id="searchform" action="<?php echo home_url('/'); ?>" class="relative">
                        ...
                        <button type="submit" id="searchsubmit" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer hover:text-gray-600">
